...
function renderBrain(data) {
  ...
  if (!visibleResults.length) {
    ...
    els.brainOutput.textContent = results.length
      ? "Somut uyuşmazlıkla doğrudan bağlantılı kaynak bulunamadı; konu dışı kaynaklar dilekçede kullanılmayacak."
      : "Kaynak sonucu bulunamadı.";
    return;
  }
  els.brainOutput.className = "result-list";
  els.brainOutput.innerHTML = visibleResults
    .map((item) => {
      ...
      return `
        <div class="result-item">
          ...
          <div class="meta-row">
            ...
            ${item.is_directly_relevant === false ? `<span class="chip">Dolaylı / denetle</span>` : ""}
...
function cleanDecisionParagraph(item) {
  ...
  return cleanOutputText(raw, risk ? RISK_PRECEDENT_PARAGRAPH : DEFAULT_PRECEDENT_PARAGRAPH);
}
...
function renderRisks({ caseEnrichment = lastCaseEnrichment, draftAudit = null, sourceAudit = null, precedentAudit = null, draftWarnings = [] } = {}) {
  const cards = [];
  const missing = [...(caseEnrichment?.missing_facts || []), ...(draftAudit?.missing_facts || [])];
  const riskFlags = [...(caseEnrichment?.risk_flags || []), ...(draftAudit?.critical_issues || []), ...(draftAudit?.major_issues || [])];
  const sourceProblems = [
    ...(draftAudit?.source_problems || []),
    ...((sourceAudit?.audited_sources || [])
      .filter((item) => !item.use_in_petition)
      .map((item) => item.source_rejected_reason || item.reason)
      .filter(Boolean)),
  ];
  const precedentProblems = [
    ...(draftAudit?.precedent_problems || []),
    ...((precedentAudit?.audited_precedents || [])
      .filter((item) => item.alignment === "riskli" || item.alignment === "aleyhe")
      .map((item) => item.risk_reason || RISK_PRECEDENT_PARAGRAPH)
      .filter(Boolean)),
  ];
...
async function runAuditPrecedents() {
  ...
  try {
    ...
    lastDecisions = lastDecisions
      .map((decision) => {
        const id = plainText([decision.court || "Yargıtay", decision.esas_no || "", decision.karar_no || "", decision.date || ""].join(" | "));
        const item = auditMap.get(id);
        return item
          ? {
              ...decision,
    ...
    renderDecisions({ top_decisions: lastDecisions });
...
async function runFullReview() {
  ...
  try {
    ...
    lastDecisions = lastDecisions
      .map((decision) => {
        const id = plainText([decision.court || "Yargıtay", decision.esas_no || "", decision.karar_no || "", decision.date || ""].join(" | "));
        const item = precedentAuditMap.get(id);
        return item
          ? {
              ...decision,
              lehe_aleyhe: item.alignment,
    ...
    setStatus("9/11 Dilekçe taslağı hazırlanıyor...");
    draft = await apiPost("/petition/draft", {
      case_text: caseText,
      answers: buildAnswers(),
      selected_decisions: mapSelectedDecisions(),
      request_type: getRequestType(),
    ...
    setStatus("10/11 Dilekçe kalite kontrolü yapılıyor...");
    ...
    if (draftAudit.can_refine && !(draftAudit.critical_issues || []).length) {
      setStatus("11/11 Usta avukat redaksiyonu deneniyor...");
      ...
      setStatus("11/11 Kritik uyarı nedeniyle redaksiyon atlandı.");
    }

    renderRisks({ caseEnrichment: enrichment, sourceAudit, precedentAudit, draftAudit, draftWarnings: draft?.warnings || [] });
    renderReviewSummary({
      analysis,
      enrichment,
      sourceCount: lastBrainResults.length,
      precedentCount: lastDecisions.length,
      qualityScore: draftAudit?.quality_score ?? null,
    });
    switchTab("petition");
    setStatus("Emsalist inceleme tamamlandı.");
  } finally {
    setBusy(false);
  }
}
