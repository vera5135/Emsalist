...
class PetitionDraftService:
    ...
    def _defective_vehicle_fact_section(self, *, case_text: str, answers: dict[str, str]) -> str:
        ...
        sale = self._select_narrative(
            ...
            f"Müvekkil, dosyaya yansıyan anlatıma göre aracı satın almış; satış ilişkisi, satış bedeli ve satıcının satış öncesi beyanları dosyadaki belgelerle ortaya konulacaktır. Olay anlatımı: {source_text}.",
        )
        defect = self._select_narrative(
            source_text,
            (
                "gizli ayip",
                "ayip",
            ...
            "Araç tesliminden sonra olağan muayene ile fark edilmesi beklenmeyen ayıp emareleri ortaya çıkmış; ayıbın satış anında mevcut olup olmadığı servis kayıtları, ekspertiz raporu, hasar kayıtları ve bilirkişi incelemesiyle açıklığa kavuşturulacaktır.",
        )
        notice = self._select_narrative(
            source_text,
            (
                "ihbar",
                "bildirim",
            ...
            "Müvekkil, ayıbı öğrendikten sonra satıcıya başvurarak ayıptan doğan haklarını kullanmak istemiştir; bildirim tarihi ve yöntemi dosyadaki yazışma, ihtarname veya diğer kayıtlarla somutlaştırılacaktır.",
        )
        demand = self._select_narrative(
            source_text,
            (
                "sozlesmeden donme",
                "bedel indirimi",
            ...
            "Müvekkilin öncelikli talebi sözleşmeden dönme ve satış bedelinin iadesidir; mahkeme aksi kanaatte olursa ayıp oranında bedel indirimi ile onarım, ekspertiz, servis ve sair zarar kalemlerinin tahsili talep edilmektedir.",
        )
        return "\n".join(
            [
                "II. MADDİ VAKIALAR",
                "A. Satış ilişkisi ve beyanlar",
                f"1. {sale}",
                "B. Ayıbın ortaya çıkışı",
                f"2. {defect}",
                "C. Ayıp ihbarı",
                f"3. {notice}",
                "D. Talep ve zarar kalemleri",
                f"4. {demand}",
            ]
        )
