"""P1.5.3 — JWT aud/nbf enforcement, DB-backed sessions and production checks."""
...
async def resolve_current_user(
    ...
    async with sessionmaker() as db:
        ...
        if auth_session is None:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=_UNAUTHORIZED)
        if auth_session.user_id != sub or auth_session.tenant_id != tenant_id:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=_UNAUTHORIZED)

    ctx.authenticated = True
    ctx.actor_id = user.id
    ctx.tenant_id = user.tenant_id
    ctx.role = user.role or "lawyer"
