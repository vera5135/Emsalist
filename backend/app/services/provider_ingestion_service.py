"""P2.6C ÔÇö Official provider ingestion orchestration.
...
does so exclusively through the P2.6 ``ingest_official_fetch`` path:
    ...
    provider.discover  ÔåÆ ProviderDiscoveryCandidate (UNTRUSTED)
    provider.fetch     ÔåÆ FetchResult via SSRF-validated source_fetcher
    provider.parse     ÔåÆ ParsedOfficialSource (canonical fields only)
    quality gate       ÔåÆ reject empty / UI-only bodies
    ingest_official_fetch(fetch_result)  ÔåÆ P2.6 trust from exact fetched bytes
...
async def _fetch_parse_ingest(db, provider, cand, *, transport, resolver):
    ...
    # Extraction provenance: raw HTML → legal text → SourceVersion
    extracted = extract_content_from_fetch(
        fetch_result, source_type=cand.source_type, parser_version="p2.6c-extract-1")
